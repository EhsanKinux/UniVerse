...
import type { NewsAttachment } from "@/lib/api/types";
...
/**
 * One downloadable attachment on the news detail page. Mirrors the documents
 * feature's view/download actions (see courses page): tap مشاهده to open inline,
 * دانلود to save with the original filename.
 */
export function NewsAttachmentCard({ attachment }: { attachment: NewsAttachment }) {
  return (
    <div className="flex items-center gap-3 rounded-2xl border border-border bg-card/70 p-3.5">
      ...
      <div className="flex shrink-0 items-center gap-1.5">
        <IconLink href={newsApi.attachmentUrl(attachment.id)} icon={ViewIcon} title="مشاهده" />
        <IconLink
          href={newsApi.attachmentUrl(attachment.id, { download: true })}
          icon={Download04Icon}
          title="دانلود"
        />
      </div>
