...
import type { DormAnnouncementAttachment } from "@/lib/api/types";
...
/** One downloadable attachment on the dorm announcement detail page. Mirrors
 *  NewsAttachmentCard: tap مشاهده to open inline, دانلود to save. */
export function DormAttachmentCard({ attachment }: { attachment: DormAnnouncementAttachment }) {
  return (
    <div className="flex items-center gap-3 rounded-2xl border border-border bg-card/70 p-3.5">
      ...
      <div className="flex shrink-0 items-center gap-1.5">
        <IconLink href={dormApi.attachmentUrl(attachment.id)} icon={ViewIcon} title="مشاهده" />
        <IconLink
          href={dormApi.attachmentUrl(attachment.id, { download: true })}
          icon={Download04Icon}
          title="دانلود"
        />
      </div>
