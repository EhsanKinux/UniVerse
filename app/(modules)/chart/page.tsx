...
import { EmptyState, InfoNote, SearchBox, SectionHeading } from "@/components/module/module-ui";
...
export default function ChartPage() {
  ...
  const totalPdfs = useMemo(() => departments.reduce((sum, dept) => sum + dept.pdfs.length, 0), []);
  ...
  const filteredDepartments = useMemo(() => {
    ...
    return departments.filter(
      (dept) =>
        ...
        dept.pdfs.some((pdf) => pdf.title.includes(query) || pdf.badge?.includes(query)),
    );
  }, [searchQuery]);
  ...
  return (
    <div className="space-y-6">
      <ModuleHero
        ...
        tone={HERO_TONE}
        stats={[
          { icon: BookOpen01Icon, value: String(departments.length), label: "رشته تحصیلی" },
          { icon: SparklesIcon, value: String(totalPdfs), label: "چارت آموزشی" },
        ]}
      />

      <SearchBox value={searchQuery} onChange={setSearchQuery} placeholder="جستجوی رشته یا چارت آموزشی..." />
