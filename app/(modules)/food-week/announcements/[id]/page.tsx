...
/** One food announcement's detail page — mirrors the dorm announcement page. */
export default function FoodAnnouncementDetailPage() {
  const params = useParams<{ id: string }>();
  const id = params?.id ?? "";
  const { data, isLoading, isError, refetch } = useFoodAnnouncement(id);
            ...
            <section className="space-y-3 border-t border-border pt-5">
              ...
              <div className="grid gap-2.5 md:grid-cols-2">
                {data.attachments.map((attachment) => (
                  <FoodAttachmentCard key={attachment.id} attachment={attachment} />
                ))}
              </div>
            </section>
          )}
